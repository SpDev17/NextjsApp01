...
const AdvancedTable3: React.FC<TableProps> = ({ columns, data, model }) => {
    const [loading, setLoading] = useState(true);
    const [sortColumn, setSortColumn] = useState<string | null>(model[0].sort);
    ...
    const [modeldata, setModeldata] = useState<TableColumn[]>([]);

    const [apiResponse, setApiResponse] = useState(null);

    const fetchData = async (searchTerm: string, sortColumn: any, isDesc: boolean, page: number, limit: number) => {
        console.log('Fetching data : searchTerm-' + searchTerm + ', sortColumn-' + sortColumn + ', isDesc-' + isDesc + ', page-' + page + ', limit-' + limit);
        try {
            let selectArray: any = [];
            if (columns && columns.length > 0) {
                for (let index = 0; index < columns.length; index++) {
                    const element = columns[index];
                    if (element.defaultload) {
                        selectArray.push(element.key);
                    }
                }
            }
            var qStr = {
                select: JSON.stringify(selectArray),
                search: JSON.stringify(searchTerm),
                sort: JSON.stringify(sortColumn),
                desc: JSON.stringify(isDesc),
                page: page,
                limit: limit,

            };
            debugger;
            let reqUrl1: any = process.env.NEXT_PUBLIC_Host;
            console.log('url : ' + reqUrl1);
            let reqUrl: string = process.env.NEXT_PUBLIC_SERVER_BASE_URL+'/api/data/user/get_json?' + queryString.stringify(qStr);


            //const response = await fetch(process.env.NEXT_PUBLIC_SERVER_BASE_URL+'/api/data/user/get_json??desc=false&limit=10&lookup=%5B%7B%22model%22%3A%22group%22%2C%22select%22%3A%22title%22%2C%22path%22%3A%22groups%22%7D%5D&page=1&search=%22%22&select=%5B%22first_name%22%2C%22last_name%22%2C%22email%22%2C%22mobile%22%2C%22isactive%22%5D&sort=%22email%22');
    ...
    useEffect(() => {
        ...
        fetchData(searchTerm, sortColumn, false, 1, 10);
    }, []);
    ...
    const handleSort = (columnKey: string) => {
        ...
        if (sortColumn === columnKey) {
            isDesc = sortDirection === 'asc' ? true : false;
        } else {
            isDesc = false;
        }
        fetchData(searchTerm, columnKey, isDesc, 1, 10);
        /*
        if (sortColumn === columnKey) {
    ...
    return (
        <div className="flex items-top justify-left p-3 md:px-1 md:py-1 bg-purple-100 rounded-lg w-full">
            {loading ? (
                <p>Loading...</p> // Display loading message or image while fetching data
            ) : (
                <section className="bg-red sm:p-1 w-full">
                    <div className="mx-auto max-w-screen-xl">

                        <div className="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                            <div className="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4 w-full bg-purple-200">
                                        ...
                                        {isActionOpen && (
                                            ...
                                            <div className="absolute right-0 z-10 mt-2 w-36 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabIndex={-1}>
                                                <div className="py-1" role="none">
                                                    ...
                                                    <a onClick={refresh} className="block px-4 py-2 text-sm text-gray-700 hover:cursor-pointer" role="menuitem" tabIndex={-1} id="menu-item-2">Refresh</a>
                                ...
                                <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                    ...
                                    <tbody>
                                        {checkedItems && checkedItems.map((item, index) => (
                                            <tr key={index} className="border-b dark:border-gray-700 transition duration-300 ease-in-out hover:bg-purple-200">
                            ...
                            <nav className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4" aria-label="Table navigation">
                                <span className="text-sm font-normal text-gray-500 dark:text-gray-400">
                                    ...
                                    of
                                    <span className="font-semibold text-gray-900 dark:text-white">{apiResponse ? apiResponse['total'] : ''}</span>
                                </span>
