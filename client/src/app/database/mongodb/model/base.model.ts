...
function isValidPassword(user: any, password: string) {
    ...
    //let encrypt_pass: any = Cryprto.AES.encrypt(password, Secrets_session).toString();
    let decrypt_pass: any = Cryprto.AES.decrypt(user.password, Secrets_session);
    decrypt_pass = decrypt_pass.toString(Cryprto.enc.Utf8);
    //console.log('Password check : ' + encrypt_pass + ',' + user.password + ', decrypt passwordv :' + decrypt_pass);
...
function createHash(password: string) {
    return Cryprto.AES.encrypt(password, Secrets_session).toString();
